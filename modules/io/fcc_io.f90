    ...
    subroutine generic_natoms_reader(unt,filetype,Nat,error_flag)
        ...
        select case (adjustl(filetype))
            ...
            case("orca4")
             call read_orca_natoms(unt,Nat,error_flag)
            case("engrad")
             call read_orcaengrad_natoms(unt,Nat,error_flag)
            case("hess")
             call read_orca_natoms(unt,Nat,error_flag)
            case("qchem")
...
!         integer,dimension(:),intent(out):: AtNum (TODO)
        ...
        select case (adjustl(filetype))
            ...
            case("orca4")
             ...
             call assign_masses(Nat,AtName,Mass,error_flag)
            case("engrad")
             call read_orcaengrad_geom(unt,Nat,AtName,X,Y,Z,Mass,error_flag)
             call assign_masses(Nat,AtName,Mass,error_flag)
            case("hess")
             call read_orca_geom(unt,Nat,AtName,X,Y,Z,Mass,error_flag)
            case("qchem")
...
!              deallocate(section)
            case("fchk")
             ...
             deallocate(A)
            case("engrad")
             call read_orcaengrad_ener(unt,E,error_flag)
!             case("cfour")
    ...
    subroutine generic_gradient_reader(unt,filetype,Nat,Grad,error_flag)
        ...
        select case (adjustl(filetype))
            ...
            case("molcas")
             call read_molcas_grad(unt,Nat,Grad,error_flag)
            case("engrad")
             call read_orcaengrad_grad(unt,Nat,Grad,error_flag)
            case("qchem")
...
!              call read_cp2k_grad(unt,Nat,Grad,error_flag)
    ...
    subroutine generic_dipir_reader(unt,filetype,Si,Dip,DipD,&
        ...
        select case (adjustl(filetype))
            ...
            case("hess")
             Dip=0.d0
             call alert_msg('note','Dipole not present in file. Set to 0')
             call read_orcahess_irdip(unt,DipD,error_flag)
            case default
             write(0,*) "Unsupported filetype:"//trim(adjustl(filetype))
             call supported_filetype_list('freq')
             error_local = 99
